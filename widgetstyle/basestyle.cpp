...
// 澶勭悊 accentColor 鍙樺寲鐨勬Ы鍑芥暟
void onAccentColorChanged() {
    QDBusInterface iface("org.lingmo.Settings",
                         "/Theme",
                         "org.lingmo.Theme",
...
void initializeBaseColor(QObject *context) {
    ...
    if (iface.isValid()) {
        ...
        // 杩炴帴 accentColorChanged 淇″彿鍒版Ы鍑芥暟
        QDBusConnection::sessionBus().connect("org.lingmo.Settings",
                                              "/Theme",
                                              "org.lingmo.Theme",
                                              "accentColorChanged",
                                              context,
                              ...
                              const QWidget* widget) const
{
    ...
    case PE_PanelLineEdit: {
        ...
        if (!enabled || isReadOnly) {
            ...
            borderColor = baseColor;  // macOS 鐒︾偣鏃剁殑钃濊壊
        } else if (mouseOver) {
        ...
        //     QColor glowColor = QColor("#007aff");  // macOS 鐒︾偣鏃剁殑钃濊壊
        //     glowColor.setAlpha(50);
            
        //     painter->setPen(Qt::NoPen);
        //     painter->setBrush(glowColor);
        //     painter->drawRoundedRect(rect.adjusted(-2, -2, 1, 1), 8, 8);  // 鎵╁睍鍙戝厜鍖哄煙
        // }
    ...
    case PE_IndicatorRadioButton: {
        ...
        // 鑾峰彇鎸囩ず鍣ㄧ煩褰?- 浣跨敤 subElementRect 鑰屼笉鏄?subControlRect
        ...
        if (checked) {
            ...
            // 璋冩暣鍐呴儴鐐圭殑澶у皬鍜屼綅缃互纭繚灞呬腑
            qreal margin = diameter * 0.35;  // 璋冩暣鐐圭殑澶у皬姣斾緥
                            ...
                            const QWidget* widget) const
{
    ...
    case CE_RadioButton: {
        ...
        if (isRadio) {
            ...
            QRect smallerCheckRect = checkRect.adjusted(
                checkRect.width() * 0.05,   // 宸﹁竟鐣屽悜鍙宠皟鏁?%
                checkRect.height() * 0.05, // 涓婅竟鐣屽悜涓嬭皟鏁?%
                -checkRect.width() * 0.05, // 鍙宠竟鐣屽悜宸﹁皟鏁?%
                -checkRect.height() * 0.05 // 涓嬭竟鐣屽悜涓婅皟鏁?%
            );
            ...
            if (isChecked && isEnabled) {
                // 鍐呴儴鍦嗙偣澶у皬涓哄鍦堢殑50%
                int dotSize = qRound(smallerCheckRect.width() * 0.5);  // 鍐呭渾澶у皬涓虹缉灏忓悗澶栧渾瀹藉害鐨?0%

                // 浣跨敤鍐呭渾鐨勪腑蹇冪偣鏉ョ粯鍒?
                QPointF dotCenter = smallerCheckRect.center();  // 鑾峰彇澶栧渾鐨勪腑蹇冪偣
                QRectF dotRect(
                    dotCenter.x() - dotSize / 2.5,  // 宸︿笂瑙抶鍧愭爣
                    dotCenter.y() - dotSize / 2.5,  // 宸︿笂瑙抷鍧愭爣
                    dotSize, dotSize               // 瀹藉害鍜岄珮搴?
                );
            ...
            if (isChecked) {
                ...
                painter->setPen(QPen(borderColor, 1)); // 鍑忓皬杈规瀹藉害
                painter->setBrush(bgColor);
    ...
    case CE_PushButtonLabel: {
        ...
        if (!button->icon.isNull() || !button->text.isEmpty()) {
            ...
            // 閲嶆柊璁＄畻璧峰浣嶇疆锛屼娇鏁翠綋灞呬腑
            int startX = rect.x() + (rect.width() - totalWidth) / 2;
            
            // 濡傛灉鏈夊浘鏍囷紝閲嶆柊璋冩暣鍥炬爣浣嶇疆
            if (!button->icon.isNull()) {
    ...
    case CE_ProgressBar: {
        ...
        if (!isIndeterminate && !filled.isEmpty()) {
            ...
            // 璁剧疆娓愬彉棰滆壊 - 浠庡熀纭€棰滆壊閫愭笎鍙樹寒
            gradient.setColorAt(0, baseColor);
            gradient.setColorAt(1, baseColor.lighter(115)); // 缁撴潫鐐瑰彉浜?5%
        ...
        if (progressBar->textVisible) {
            ...
            // 鏍规嵁杩涘害鏉＄殑閫変腑鐘舵€佽缃枃瀛楅鑹?
            QColor textColor;
            if (progressBar->state & State_Selected) {
                textColor = swatch.color(S_highlightedText);
            } else {
                if (filled.contains(rect.center())) {
                    textColor = Qt::white;  // 鍦ㄥ～鍏呭尯鍩熷唴浣跨敤鐧借壊
                } else {
                    textColor = isDarkMode() ? Qt::white : Qt::black;  // 鍦ㄦ湭濉厖鍖哄煙鏍规嵁涓婚浣跨敤瀵瑰簲棰滆壊
                }
            }
    ...
    case CE_TabBarTab: {
        ...
        if (!enabled) {
            ...
            bgColor = baseColor;
            if (pressed) {
                bgColor = baseColor.darker(110);  // 閫変腑鐘舵€佷笅鎸夊帇鍙樻殫10%
            } else if (hovered) {
                bgColor = baseColor.lighter(110);  // 閫変腑鐘舵€佷笅鎮仠鍙樹寒10%
            }
        } else {
            if (pressed) {
                bgColor = baseTabColor.darker(110);  // 鏈€変腑鐘舵€佷笅鎸夊帇鍙樻殫10%
            } else if (hovered) {
                bgColor = baseTabColor.lighter(110);  // 鏈€変腑鐘舵€佷笅鎮仠鍙樹寒10%
            } else {
                bgColor = baseTabColor;
            }
        }
        ...
        // 缁樺埗鍒嗛殧绾匡紙浠呭湪鏈€変腑涓旀湭鎮仠涓旀湭鎸夊帇鏃讹級
        ...
        if (!tab->text.isEmpty()) {
            ...
            if (!enabled) {
                ...
                if (pressed) {
                    textColor.setAlpha(180);  // 鎸夊帇鏃舵枃鏈洿閫忔槑
                } else if (hovered) {
                    textColor = textColor.lighter(110);  // 鎮仠鏃舵枃鏈彉浜?
                }
            }
                                   ...
                                   const QWidget* widget) const
{
    ...
    case CC_SpinBox: {
        ...
        painter->setPen(QPen(borderColor, 1));
        painter->setBrush(bgColor);
        painter->drawRoundedRect(rect.adjusted(0, 0, -1, -1), 11, 11); // 淇敼鍦嗚涓?1
    ...
    case CC_ComboBox: {
        ...
        painter->save();
        painter->setRenderHint(QPainter::Antialiasing);
        painter->setRenderHint(QPainter::TextAntialiasing);  // 娣诲姞鏂囨湰鎶楅敮榻?
    ...
    case CC_ToolButton: {
        ...
        // 鍒ゆ柇鏄剧ず鍐呭
        bool hasIcon = !tbopt->icon.isNull();  // 鏄惁鏈夊浘鏍?
        bool hasText = !tbopt->text.isEmpty(); // 鏄惁鏈夋枃瀛?
